...
function Mesthiri() {
  const [mesthiris, setMesthiris] = useState([])
  ...
  const [editingMesthiri, setEditingMesthiri] =
    useState(null)

  const [showDelete, setShowDelete] = useState(false)
  const [deletingMesthiri, setDeletingMesthiri] =
    useState(null)

  const [showAssignment, setShowAssignment] =
    useState(false)

  const [assignmentMesthiri, setAssignmentMesthiri] =
    useState(null)
  ...
  const loadData = async () => {
    try {
      ...
      const [mesthiriResponse, siteResponse] =
        await Promise.all([
          api.get("/mesthiri/"),
          api.get("/sites/"),
        ])
      ...
      setMesthiris(
        Array.isArray(mesthiriResponse.data)
          ? mesthiriResponse.data
          : mesthiriResponse.data?.results || []
      )
  ...
  const openEdit = (mesthiri) => {
    setEditingMesthiri(mesthiri)
    setForm({
      name: mesthiri.name || "",
    })
    setError("")
  ...
  const handleSave = async (event) => {
    ...
    if (!form.name.trim()) {
      setError("Please enter the mesthiri name.")
      return
    }
    ...
    try {
      ...
      if (editingMesthiri) {
        ...
        setSuccess(
          "Mesthiri updated successfully."
        )
      } else {
        await api.post("/mesthiri/", {
          name: form.name.trim(),
        })

        setSuccess(
          "Mesthiri added successfully."
        )
      }
  ...
  const openDelete = (mesthiri) => {
    setDeletingMesthiri(mesthiri)
    setError("")
    setShowDelete(true)
  }
  ...
  const closeDelete = () => {
    if (saving) return
    setShowDelete(false)
    setDeletingMesthiri(null)
  }

  const handleDelete = async () => {
    if (!deletingMesthiri) return
    ...
    try {
      ...
      await api.delete(
        `/mesthiri/${deletingMesthiri.id}/`
      )

      setSuccess(
        "Mesthiri deleted successfully."
      )
  ...
  const getCurrentSite = (mesthiri) => {
    return (
      mesthiri.current_site_name ||
      mesthiri.current_site?.name ||
      mesthiri.current_site?.site_name ||
      null
    )
  }
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto w-full max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-500">
              Manage your complete mesthiri master list,
        ...
        <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-3">
          <Stat
            label="Total Mesthiri"
            value={mesthiris.length}
            icon={HardHat}
          />
          ...
          {loading ? (
            ...
            <div className="rounded-3xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
              ...
              <h2 className="mt-4 text-lg font-bold text-slate-900">
                No mesthiri added yet
              </h2>

              <p className="mx-auto mt-1 max-w-md text-sm text-slate-500">
                Add your mesthiri once here and assign
                them to sites whenever their work changes.
              </p>
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="divide-y divide-slate-100">
                {mesthiris.map((mesthiri) => {
                  const currentSite =
                    getCurrentSite(mesthiri)
        ...
        <Modal onClose={closeForm}>
          <div className="p-5 sm:p-6">
            <ModalHeader
              ...
              subtitle={
                editingMesthiri
                  ? "Update the mesthiri master record."
                  : "Create a mesthiri record for your workforce."
              }
              onClose={closeForm}
            />
            ...
            >
              <label className="block">
                ...
                <input
                  ...
                  placeholder="Enter mesthiri name"
      ...
      {/* DELETE */}
      {showDelete && deletingMesthiri && (
        <Modal onClose={closeDelete}>
          <div className="p-5 sm:p-6">
            <ModalHeader
              title="Delete Mesthiri?"
      ...
      {showAssignment &&
        assignmentMesthiri && (
          <Modal onClose={closeAssignment}>
            <div className="p-5 sm:p-6">
              <ModalHeader
                title="Assign Mesthiri"
                subtitle={`Assign ${assignmentMesthiri.name} to a site.`}
              ...
              >
                ...
                <div className="rounded-2xl bg-slate-50 p-4 text-xs leading-5 text-slate-500">
                  The assignment date determines which
                  site this mesthiri belongs to for daily
                  entries.
                </div>
